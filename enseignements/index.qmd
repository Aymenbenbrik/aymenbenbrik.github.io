...
def render_resource_button(name, status, link):
    s = (status or "").lower()
    if "à venir" in s or not link or link == "":
...
parts = []
for d, icon, desc in DOMAINS:
    d_df = df_sup[df_sup["domaine"] == d]
    if d_df.empty:
        parts.append(f'''
<details class="callout" style="margin: 1rem 0;">
  <summary style="cursor:pointer; padding: 0.8rem 1rem; background:#FAFAFA; border-left: 4px solid #C8102E; font-weight:600;">
    {icon}&nbsp; <strong>{d}</strong>&nbsp; <span style="color:#999; font-weight:400; font-size:0.9em">— pas encore de supports en ligne</span>
  </summary>
</details>''')
        continue
    ...
    n_modules = d_df["nom_module"].nunique()
    cards = []
    for (mod, niv, classe, ecole), g in d_df.groupby(
        ["nom_module", "niveau", "classe", "ecole"], sort=False
    ):
        rows_html = []
...
<div class="module-card">
  <h4 class="module-title">{escape(mod)}</h4>
  <div class="module-meta">
    <span>{escape(niv)}</span>
    <span>{escape(classe)}</span>
    <span>{escape(ecole)}</span>
  </div>
  <div class="resources">
    {''.join(rows_html)}
  </div>
...
<details class="callout" open style="margin: 1rem 0;">
  <summary style="cursor:pointer; padding: 0.8rem 1rem; background:#FAFAFA; border-left: 4px solid #C8102E; font-weight:600;">
    {icon}&nbsp; <strong>{d}</strong>&nbsp; <span style="color:#666; font-weight:400; font-size:0.9em">— {n_modules} module(s) avec supports</span>
  </summary>
